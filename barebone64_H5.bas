...
7010 if sp>=ls or ec<>0 then ec=2:return: rem check for stack overflow
7020 sp=sp+1:s(sp)=v
7030 return

7100 rem *** pop v from stack ***
7110 if sp<0 or ec<>0 then sp=-1:ec=1:return: rem check for stack underflow
7120 v=s(sp):sp=sp-1
7130 return
...
9030 id=50:open id,0: rem open the keyboard as device
9100 return
